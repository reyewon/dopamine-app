...
export async function GET(req: NextRequest) {
  ...
  if (!code || !clientId || !clientSecret) {
    ...
    return htmlPage('Connection failed', `Missing: ${missing}. Check environment variables in Cloudflare Pages.`, false);
  }
  ...
  const kv = getKV();
  if (!kv) {
    return htmlPage('Storage unavailable', 'KV namespace not bound. Check DOPAMINE_KV binding in Cloudflare Pages settings.', false);
  }

  try {
    await kv.put('calendar-tokens', JSON.stringify(tokens));
  } catch (err) {
    return htmlPage('Storage error', `Failed to write tokens to KV: ${err instanceof Error ? err.message : String(err)}`, false);
  }

  return htmlPage('Google Calendar connected!', 'New shoots will now be added to your Photography calendar automatically.', true);
}
